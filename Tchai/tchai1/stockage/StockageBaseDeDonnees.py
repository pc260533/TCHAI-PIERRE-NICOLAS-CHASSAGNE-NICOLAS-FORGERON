...
class StockageBaseDeDonnees(object):
    ...
    def enregistrerTransaction(self, transaction: Transaction):
        try:
            sql = "INSERT INTO transactionUtilisateurs (nomUtilisateur1, nomUtilisateur2, montantTransaction, dateTransaction) VALUES (:nomUtilisateur1, :nomUtilisateur2, :montantTransaction, :dateTransaction);";
    ...
    def chargerListeTransactions(self) -> List[Transaction]:
        try:
            ...
            sql = "SELECT nomUtilisateur1, nomUtilisateur2, montantTransaction, dateTransaction FROM transactionUtilisateurs ORDER BY dateTransaction ASC";
    ...
    def chargerListeTransactionsAvecUtilisateur(self, nomUtilisateur: str) -> List[Transaction]:
        try:
            if (not self.verifierUtilisateurExiste(nomUtilisateur)):
                raise ExceptionGestionBaseDeDonnees(None, " : l'utilisateur n'existe pas.");
            listeTransactions: List[Transaction] = [];
            sql = "SELECT nomUtilisateur1, nomUtilisateur2, montantTransaction, dateTransaction FROM transactionUtilisateurs WHERE nomUtilisateur1 = :nomUtilisateur OR nomUtilisateur2 = :nomUtilisateur ORDER BY dateTransaction ASC";
            self.cursor.execute(sql, {"nomUtilisateur": nomUtilisateur});
            lignesTransactions = self.cursor.fetchall();
            for ligneTransaction in lignesTransactions:
                transaction: Transaction = Transaction();
